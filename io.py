...
def read_nbody_file(param):
    ...
    if (nbody_file_format=='tipsy'):
        ...
        if size=='big':
            path = re.match(r'(.*/bigbox/)', nbody_file_in).group(1)
        elif size=='small':
            path = re.match(r'(.*/smallbox/)', nbody_file_in).group(1)
...
def read_halo_file(param):
    ...
    if (halo_file_format=='AHF-ASCII'):
        ...
        import numpy as np
        from astropy.io import ascii
        from miniramses.utils.py.miniramses import rd_clump, ClumpCat, Info, rd_info
        from scipy.optimize import fsolve

        if size=='big':
            path = re.match(r'(.*/bigbox/)', halo_file_in).group(1)
        elif size=='small':
            path = re.match(r'(.*/smallbox/)', halo_file_in).group(1)

        # Extracting the number as an integer
        nout = int(re.match(r'.*/output_(\d+)', halo_file_in).group(1))

        i = rd_info(nout, path=path)
        cat = rd_clump(nout, path=path)
        ...
        h_dt = np.dtype([('ID', '<i8'), ('IDhost', '<i8'), ('Mvir', '<f8'), ('x', '<f8'), ('y', '<f8'),
                         ('z', '<f8'),  ('rvir', '<f8'),  ('cvir', '<f8'), ('vx', '<f8'), ('vy', '<f8'), ('vz', '<f8')])
        ...
        h['vx'], h['vy'], h['vz'] = cat.u*unit_v_in_km_per_s, cat.v*unit_v_in_km_per_s, cat.w*unit_v_in_km_per_s

        # h['rvir'] = cat.r200 * unit_l_in_Mpc_per_h
        # h['Mvir'] = cat.mass * unit_m_in_Msol_per_h
        # h['cvir'] = cat.c200
        
        def r200c_to_r200b(xc, *args):
            Om, xb = args
            return (np.log(1+xc) - xc/(1+xc))/(np.log(1+xb) - xb/(1+xb)) * xb**3.0 / xc**3.0 - 1/Om

        def Massfunc(r, rs): #The NFW mass function without rho_s
            return 4 * np.pi * rs**3.0 * (np.log(1 + r/rs) - r/(rs + r))
            
        print('Converting r200b, M200b, and c200b to r200c, M200c, and c200c')
        for j in range(len(h['Mvir'])):
            args = (i.omega_m, cat.c200[j])
            initial_guess = cat.c200[j]
            Xc = fsolve(r200c_to_r200b, initial_guess, args=args)
            h['cvir'][j] += Xc
            h['rvir'][j] += Xc * cat.r200[j] / cat.c200[j] * unit_l_in_Mpc_per_h 
            rho_s = cat.mass[j] / Massfunc(cat.r200[j], cat.r200[j]/cat.c200[j]) #We find rho_s
            h['Mvir'][j] += rho_s * Massfunc(h['rvir'][j], cat.r200[j]/cat.c200[j]) * unit_m_in_Msol_per_h
        print('Done!')
        
        h['ID']     = cat.index
        h['IDhost'] = np.where(cat.index == cat.parent, -1.0, cat.parent)
        
        h = h[h['Mvir']>param.code.Mhalo_min]
        
        args = np.argsort(h['Mvir'])
        args = np.flip(args) #This will sort high to low
        h['x'], h['y'], h['z'] = h['x'][args], h['y'][args], h['z'][args]
        h['vx'], h['vy'], h['vz'] = h['vx'][args], h['vy'][args], h['vz'][args]
        h['Mvir'] = h['Mvir'][args]
        h['rvir'] = h['rvir'][args]
        h['cvir'] = h['cvir'][args]
        h['ID'] = h['ID'][args]
        h['IDhost'] = h['IDhost'][args]
